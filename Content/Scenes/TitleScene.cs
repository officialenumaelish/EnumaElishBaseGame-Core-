...
namespace EnumaElishMonoGameMultiplayer.Content.Scenes
{
    public class TitleScene : Scene
    {
        int _width;
        int _height;
        public TitleScene(int width, int heigth)
        {
            _width = width;
            _height = heigth;
        }
        ...
        public override void Initialize()
        {
            ...
            MonoGameLibrary.Core.ExitOnEscape = true;
        ...
        public override void LoadContent()
        {
            ...
            _font = MonoGameLibrary.Core.Content.Load<SpriteFont>("Fonts/mainfont");

            // Load the font for the title text.
            _font5x = Content.Load<SpriteFont>("Fonts/mainfont");
        }

        public override void Update(GameTime gameTime)
        {
            // If the user presses enter, switch to the game scene.
            if (MonoGameLibrary.Core.Input.Keyboard.WasKeyJustPressed(Keys.Enter))
            {
                MonoGameLibrary.Core.ChangeScene(new GameScene(_width, _height));
            }
        }
        ...
        {
            MonoGameLibrary.Core.GraphicsDevice.Clear(new Color(32, 40, 78, 255));
            ...
            MonoGameLibrary.Core.SpriteBatch.DrawString(_font5x, DUNGEON_TEXT, _dungeonTextPos + new Vector2(10, 10), dropShadowColor, 0.0f, _dungeonTextOrigin, 1.0f, SpriteEffects.None, 1.0f);

            // Draw the Dungeon text on top of that at its original position.
            MonoGameLibrary.Core.SpriteBatch.DrawString(_font5x, DUNGEON_TEXT, _dungeonTextPos, Color.White, 0.0f, _dungeonTextOrigin, 1.0f, SpriteEffects.None, 1.0f);

            // Draw the Slime text slightly offset from it is original position and
            // with a transparent color to give it a drop shadow.
            MonoGameLibrary.Core.SpriteBatch.DrawString(_font5x, SLIME_TEXT, _slimeTextPos + new Vector2(10, 10), dropShadowColor, 0.0f, _slimeTextOrigin, 1.0f, SpriteEffects.None, 1.0f);

            // Draw the Slime text on top of that at its original position.
            MonoGameLibrary.Core.SpriteBatch.DrawString(_font5x, SLIME_TEXT, _slimeTextPos, Color.White, 0.0f, _slimeTextOrigin, 1.0f, SpriteEffects.None, 1.0f);

            // Draw the press enter text.
            MonoGameLibrary.Core.SpriteBatch.DrawString(_font, PRESS_ENTER_TEXT, _pressEnterPos, Color.White, 0.0f, _pressEnterOrigin, 1.0f, SpriteEffects.None, 0.0f);
